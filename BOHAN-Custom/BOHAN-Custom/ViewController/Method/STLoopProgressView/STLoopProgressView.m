...
//    [self setFrame:CGRectChangeSize(self.frame, SELF_WIDTH, SELF_WIDTH)];
    ...
    self.layer.borderWidth = [STLoopProgressView lineWidth];
...
- (void)setupColorLayer {
    ...
    leftLayer.frame = CGRectMake(0, 0, SELF_WIDTH/2, SELF_WIDTH);
    // 分段设置渐变色
    leftLayer.locations = @[@0.3, @0.9, @1];
    leftLayer.colors = @[(id)[STLoopProgressView startColor].CGColor, (id)[STLoopProgressView endColor].CGColor];

//    [self.colorLayer addSublayer:leftLayer];
    [self.colorLayer insertSublayer:leftLayer atIndex:0];
...
- (CAShapeLayer *)generateMaskLayer {
    ...
    if ([STLoopProgressView clockWiseType]) {
        path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(SELF_WIDTH / 2, SELF_WIDTH / 2) radius:(SELF_WIDTH - [STLoopProgressView lineWidth]) / 2 startAngle:[STLoopProgressView startAngle] endAngle:[STLoopProgressView endAngle] clockwise:YES];
    } else {
        path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(SELF_WIDTH / 2, SELF_WIDTH / 2) radius:(SELF_WIDTH - [STLoopProgressView lineWidth]) / 2 startAngle:[STLoopProgressView endAngle] endAngle:[STLoopProgressView startAngle] clockwise:NO];
    }
